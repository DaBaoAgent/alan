...
## 朗读规则(normalize_for_tts,2026-09-12 用户定版,合成前自动应用,不改原稿)

- **人名间隔号连读**:`杰米·李·柯蒂斯`→`杰米李柯蒂斯`,人名中间的 · 一律去掉,不拆读;
  `--keep-dots` 可关闭此行为
- **相邻书名号补顿号**:`《A》《B》`→`《A》、《B》`,片名之间补 0.18s 停顿锚点
  (与句间停顿等长,统一被停顿逻辑归一)
...
- **呼吸消除(2026-09-12 用户定版)**:语句间隙 ≥0.05s 整段替换为 0.18s 纯静音
  ...
  词间/句间呼吸全部归零,实测间隙峰值 -90dBFS=数字零)
- **停顿跟着剧情场景走,不跟原稿自然段走(2026-09-12 用户纠正)**:先通读解说稿按
  剧情划分场景(一个时间/地点/叙事单元讲完才算边界,如钩子收尾「火光冲天。」后、
  背景讲完「当年这玩意儿多火。」后),`--scene-ends '7,16,…'` 传段落号(1 起),
  场景边界插 `--scene-gap 1.0` 秒;场景内自然段之间 0.35 秒节奏停顿
  (`--paragraph-gap 0.35`);scene-ends 里最后的段落号=全稿末段,不产生尾静音
- **免重跑改停顿**:段落音频都在 `配音\segments\`,改停顿参数/场景划分后加
  `--reuse-segments` 直接重拼,秒级完成,不必重跑 TTS
